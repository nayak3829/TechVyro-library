...
- Start the development app with `pnpm run dev`.
- The `Start application` workflow runs this command and serves the web preview on port 5000.
- Verify a production build with `pnpm run build`.

### Smart PDF background jobs

- The running Next.js process starts the bounded PDF worker through root instrumentation. Use a Reserved VM deployment when the web process must remain continuously active.
- For Autoscale deployments, add a Replit Scheduled Deployment that runs `pnpm run pdf-worker:once` on the shortest supported cadence. This drains due processing, notification, and orphan-cleanup jobs even when the web deployment has scaled to zero.
- Job leases and notification claims make concurrent web and scheduled workers safe.

### Environment

The public Supabase URL and anonymous key are required for browsing and authentication. Admin database operations additionally require `SUPABASE_SERVICE_ROLE_KEY`. AI and Telegram features are optional and require `OPENAI_API_KEY` and `TELEGRAM_BOT_TOKEN`, respectively. Store all sensitive values as Replit Secrets rather than committing them to project files.
