    ...
    try {
      ...
      if (error) {
        if (error.message.includes("already registered")) {
          toast.error("Yeh email pehle se registered hai. Login karo.")
          setMode("login")
        } else {
          toast.error(error.message)
        }
      } else {
        toast.success("Account ban gaya! Email verify karo agar mail aaya ho.")
        onClose()
      }
    } finally {
      setLoading(false)
  ...
  async function handleForgot(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) {
        ...
        setForgotSent(true)
        toast.success("Password reset email bheja gaya!")
      }
    } finally {
      setLoading(false)
    }
        ...
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-0.5">
                {mode === "login" ? "Apne account mein login karo" : mode === "signup" ? "Free account banao" : "Email pe link bhejenge"}
              </p>
            </div>
          ...
          {mode === "forgot" && forgotSent ? (
            <div className="text-center py-6 space-y-4">
              ...
              <div>
                <p className="font-semibold text-foreground">Email Bheja Gaya!</p>
                <p className="text-sm text-muted-foreground mt-1">Apna inbox check karo aur reset link pe click karo.</p>
              </div>
            ...
            <form onSubmit={mode === "login" ? handleLogin : mode === "signup" ? handleSignup : handleForgot} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder={mode === "signup" ? "Kam se kam 6 characters" : "Apna password likhो"}
                      value={password}
              ...
              >
                {loading ? (
                  ...
                  <><UserPlus className="h-4 w-4" />Account Banao</>
                ) : (
                  <><Mail className="h-4 w-4" />Reset Link Bhejo</>
                )}
              </Button>
            </form>
          )}
