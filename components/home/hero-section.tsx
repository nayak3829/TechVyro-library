...
const features = [
  { icon: Shield, label: "Organized Content", color: "text-primary" },
  { icon: Zap, label: "Quick Downloads", color: "text-green-500" },
  { icon: GraduationCap, label: "Study Resources", color: "text-accent" },
]
...
export function HeroSection() {
  ...
  const [taglines, setTaglines] = useState(DEFAULT_HERO_SETTINGS.taglines)
  const [trustStatLabels, setTrustStatLabels] = useState(DEFAULT_HERO_SETTINGS.trustStats)
  const [badgeText, setBadgeText] = useState(DEFAULT_HERO_SETTINGS.badgeText)
  const [description, setDescription] = useState(DEFAULT_HERO_SETTINGS.description)
  const [heroBtnText, setHeroBtnText] = useState(DEFAULT_HERO_SETTINGS.heroBtnText)
  const [whatsappBtnText, setWhatsappBtnText] = useState(DEFAULT_HERO_SETTINGS.whatsappBtnText)
  ...
  useEffect(() => {
    fetch("/api/site-settings?key=hero_settings")
      ...
      .then(data => {
        if (data.value) {
          const settings = normalizeHeroSettings(data.value)
          setTaglines(settings.taglines)
          setTrustStatLabels(settings.trustStats)
          setBadgeText(settings.badgeText)
          setDescription(settings.description)
          setHeroBtnText(settings.heroBtnText)
          setWhatsappBtnText(settings.whatsappBtnText)
        }
      })
      .catch(() => {})
    ...
    fetch("/api/site-settings?key=general_settings")
      .then(r => r.json())
      .then(data => {
        const candidate = data.value?.whatsappChannelUrl
        if (typeof candidate === "string" && isSafeHttpUrl(candidate, true)) setWhatsappUrl(candidate)
      })
      .catch(() => {})
  ...
  const displayRating = liveStats && liveStats.avgRating > 0 ? liveStats.avgRating.toFixed(1) : null

  const newUploadTime = liveStats?.latestUpload ? formatSince(liveStats.latestUpload.created_at) : null
            ...
            <div className="relative w-full max-w-[420px] mx-auto">
              ...
              <div className="relative bg-card/80 backdrop-blur-2xl border border-border/60 rounded-2xl p-5 lg:p-6 shadow-2xl shadow-black/8 ring-1 ring-white/5">
                ...
                <div className="space-y-2">
                  ...
                  {liveStats && floatingCards.length === 0 && (
                    <p className="py-6 text-center text-xs text-muted-foreground">No PDFs published yet</p>
                  )}
                </div>
                ...
                <div className="mt-4 pt-3.5 border-t border-border/40 flex items-center justify-between">
                  <div className="flex items-center gap-0.5">
                    {displayRating && <Star className="h-3.5 w-3.5 text-amber-400 fill-amber-400" />}
                    <span className="text-xs font-bold text-foreground ml-1.5">{displayRating ?? "Not yet rated"}</span>
                  </div>
              ...
              {newUploadTime && <div className="absolute -top-4 -right-4 bg-card/95 backdrop-blur-xl border border-border/60 rounded-xl px-3 py-2 shadow-xl hidden sm:flex items-center gap-2 ring-1 ring-white/5" style={{ animation: "float1 4s ease-in-out infinite" }}>
              ...
              {weekDownloadsLabel && <div className="absolute -bottom-4 -left-4 bg-card/95 backdrop-blur-xl border border-border/60 rounded-xl px-3 py-2 shadow-xl hidden sm:flex items-center gap-2 ring-1 ring-white/5" style={{ animation: "float2 5s ease-in-out infinite" }}>
