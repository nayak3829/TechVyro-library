...
export function PDFGrid({ pdfs, categories, initialSearch = "", totalPdfs = pdfs.length }: PDFGridProps) {
  ...
  const CategoryCard = ({ category, isSelected, count, onClick }: {
    ...
    <button
      ...
      className="min-w-0 text-left rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2"
    >
      <Card
      ...
      >
        <CardContent className="p-3 sm:p-4 text-center">
          ...
          <div className="flex items-center justify-center gap-1 text-[10px] sm:text-xs text-muted-foreground mt-0.5">
            <span>{count} {isPartialCatalogue ? "shown" : "files"}</span>
            {category && (
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              {totalPdfs} documents available
              {totalPdfs > pdfs.length && ` · showing latest ${pdfs.length}`}
            </p>
          </div>
        </div>
        {isPartialCatalogue && (
