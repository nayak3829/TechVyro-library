...
function normalizeQuestion(question: unknown): Question {
  const q = question && typeof question === "object" ? question as Record<string, unknown> : {}
  const type: QuestionType = q.type === "truefalse" || q.type === "multiselect" ? q.type : "mcq"
  const options = asArray<unknown>(q.options).map(option => String(option))
  return {
    id: typeof q.id === "string" && q.id ? q.id : generateId(),
    type,
    question: typeof q.question === "string" ? q.question : "",
    options: options.length ? options : (type === "truefalse" ? ["True", "False"] : ["", "", "", ""]),
    correct: typeof q.correct === "number" && Number.isFinite(q.correct) ? q.correct : 1,
    correctOptions: asArray<unknown>(q.correctOptions).filter((value): value is number => typeof value === "number" && Number.isFinite(value)),
    marks: typeof q.marks === "number" && Number.isFinite(q.marks) ? q.marks : 1,
    negativeMarks: typeof q.negativeMarks === "number" && Number.isFinite(q.negativeMarks) ? q.negativeMarks : 0,
    explanation: typeof q.explanation === "string" ? q.explanation : "",
    timeLimit: typeof q.timeLimit === "number" && Number.isFinite(q.timeLimit) ? q.timeLimit : 0,
  }
}
...
function dbRowToQuiz(row: any): Quiz {
  ...
  return {
    id: typeof source.id === "string" ? source.id : generateId(), title: typeof source.title === "string" ? source.title : "Untitled Quiz", description: typeof source.description === "string" ? source.description : "",
    category: typeof source.category === "string" ? source.category : "General", timeLimit: typeof source.time_limit === "number" ? source.time_limit : 1200,
    questions: asArray(source.questions).map(normalizeQuestion),
    enabled: typeof source.enabled === "boolean" ? source.enabled : true, createdAt: typeof source.created_at === "string" ? source.created_at : new Date().toISOString(),
    tags: asArray<unknown>(source.tags).filter((tag): tag is string => typeof tag === "string"), visibility: source.visibility === "private" || source.visibility === "unlisted" ? source.visibility : "public",
    section: typeof source.section === "string" ? source.section : "General", difficulty: source.difficulty === "easy" || source.difficulty === "hard" ? source.difficulty : "medium",
    structureLocation: source.structure_location && typeof source.structure_location === "object" ? source.structure_location as Quiz["structureLocation"] : undefined,
    negativeMarking: typeof source.negative_marking === "number" ? source.negative_marking : 0,
    passingPercentage: typeof source.passing_percentage === "number" ? source.passing_percentage : 0,
    shuffleQuestions: source.shuffle_questions === true,
    shuffleOptions: source.shuffle_options === true,
  }
}
...
export function QuizManager() {
  ...
  const [uploadEntries, setUploadEntries] = useState<{
    ...
    quiz: Quiz | null; error?: string; warning?: string
    included: boolean
  ...
  useEffect(() => {
    fetch("/api/quizzes").then(async r => {
      ...
      if (data && Array.isArray(data.quizzes)) setQuizzes(data.quizzes.map(dbRowToQuiz))
    }).catch(() => toast.error("Could not load quizzes"))
    fetch("/api/quiz-results").then(async r => {
      if (!r.ok) throw new Error("Could not load leaderboard")
      return r.json()
    }).then(data => {
      if (data && Array.isArray(data.results)) setLeaderboard(data.results.filter((e: unknown) => e && typeof e === "object").map((e: any) => ({
  ...
  useEffect(() => {
    fetch("/api/folders", { cache: "no-store" })
      .then(async response => response.ok ? response.json() : { folders: [] })
      .then(data => setContentFolders(Array.isArray(data?.folders) ? data.folders : []))
      .catch(() => setContentFolders([]))
  }, [])
  ...
  const saveQuizzes = async (updated: Quiz[]) => {
    ...
    try {
      async function ensureSuccessful(responses: Response[]) {
        ...
        const message = typeof body.error === "string"
          ...
          : failed.status === 401
            ...
            : "One or more changes could not be saved"
        throw new Error(message)
      }

      // Validate and persist creates/updates before issuing any destructive request.
      // This prevents a rejected edited quiz from being accompanied by successful deletes.
      const writeResponses = await Promise.all([
        ...toCreate.map(q => fetch("/api/quizzes", { method: "POST", headers, body: JSON.stringify({ ...q, negativeMarking: q.negativeMarking, passingPercentage: q.passingPercentage, shuffleQuestions: q.shuffleQuestions, shuffleOptions: q.shuffleOptions }) })),
        ...toUpdate.map(q => fetch(`/api/quizzes/${q.id}`, { method: "PUT", headers, body: JSON.stringify({ ...q, negativeMarking: q.negativeMarking, passingPercentage: q.passingPercentage, shuffleQuestions: q.shuffleQuestions, shuffleOptions: q.shuffleOptions }) })),
      ])
      await ensureSuccessful(writeResponses)

      const deleteResponses = await Promise.all(
        toDelete.map(q => fetch(`/api/quizzes/${q.id}`, { method: "DELETE", headers }))
      )
      await ensureSuccessful(deleteResponses)
      return true
    } catch (error) {
      try {
        const response = await fetch("/api/quizzes", { cache: "no-store" })
        const data = await response.json().catch(() => ({}))
        if (!response.ok || !Array.isArray(data.quizzes)) throw new Error("Unable to reload quizzes")
        setQuizzes(data.quizzes.map(dbRowToQuiz))
      } catch {
        setQuizzes(prev)
      }
      const message = error instanceof Error ? error.message : "Could not save quiz changes"
      toast.error(`${message}. The latest server state was reloaded.`)
      return false
    }
  }

  const handleAddQuiz = () => {
    const structureLocation = findStructureLocationByCategoryName(contentFolders, defaultQuiz.category) ?? undefined
    setEditingQuiz({ ...defaultQuiz, structureLocation, id: generateId(), createdAt: new Date().toISOString() })
    setShowQuizDialog(true)
  }

  function updateEditingQuizCategory(category: string) {
    if (!editingQuiz) return
    const match = findStructureLocationByCategoryName(contentFolders, category)
    setEditingQuiz({ ...editingQuiz, category, ...(match ? { structureLocation: match } : {}) })
  }

  function updateGlobalQuizCategory(category: string) {
    const match = findStructureLocationByCategoryName(contentFolders, category)
  ...
  const handleGenerateAiQuiz = async () => {
    ...
    try {
      ...
      if (data?.quiz && typeof data.quiz === "object" && Array.isArray(data.quiz.questions)) {
        const generatedQuiz = dbRowToQuiz({ ...data.quiz, id: generateId(), created_at: new Date().toISOString() })
        setShowAiQuizDialog(false)
        setEditingQuiz(generatedQuiz)
        setShowQuizDialog(true)
        setAiQuizTopic("")
        toast.success(`Generated ${generatedQuiz.questions.length} questions! Review and save.`)
      } else {
        toast.error("AI generation returned an invalid quiz")
      }
    } catch {
  ...
  const handleDuplicateQuiz = async (quiz: Quiz) => {
    ...
    if (!await saveQuizzes([...quizzes, dup])) return
  ...
  const handleBulkDelete = async () => {
    ...
    if (!await saveQuizzes(quizzes.filter(q => !selectedQuizzes.has(q.id)))) return
    setSelectedQuizzes(new Set()); toast.success(`Deleted ${selectedQuizzes.size} quizzes`)
  }
  const handleBulkToggle = async (enable: boolean) => {
    if (!selectedQuizzes.size) return
    if (!await saveQuizzes(quizzes.map(q => selectedQuizzes.has(q.id) ? { ...q, enabled: enable } : q))) return
  ...
  const handleBulkMove = async () => {
    ...
    if (!await saveQuizzes(quizzes.map(q =>
  ...
  const handleBulkDuplicate = async () => {
    ...
    if (!await saveQuizzes([...quizzes, ...dups])) return
  ...
  const handleSaveQuestion = async () => {
    ...
    if (!await saveQuizzes(quizzes.map(q => q.id === quizId ? { ...q, questions: updated } : q))) return
  ...
  const handleDeleteQuestion = async (quizId: string, questionId: string) => {
    ...
    if (!await saveQuizzes(quizzes.map(q => q.id === quizId ? { ...q, questions: q.questions.filter(qq => qq.id !== questionId) } : q))) return
    toast.success("Question deleted")
  }
  const handleMoveQuestion = async (quizId: string, idx: number, dir: -1 | 1) => {
    const quiz = quizzes.find(q => q.id === quizId)
    if (!quiz) return
    ...
    await saveQuizzes(quizzes.map(q => q.id === quizId ? { ...q, questions: qs } : q))
  }
  ...
  const importFromJson = async (file: File) => {
    try {
      ...
      const imported: Quiz[] = items
        .filter(q => q && typeof q === "object" && typeof q.title === "string" && Array.isArray(q.questions))
        .map(q => ({
          ...dbRowToQuiz({ ...q, id: generateId(), created_at: new Date().toISOString() }),
          questions: asArray(q.questions).map(normalizeQuestion),
        }))
      if (!imported.length) { toast.error("No valid quizzes found in JSON file"); return }
      if (!await saveQuizzes([...quizzes, ...imported])) return
  ...
  const normalizeImportedQuiz = (q: any, settings?: JsonFileEntry["settings"]): Quiz => ({
    ...
    structureLocation: q.structureLocation
      || findStructureLocationByCategoryName(contentFolders, settings?.category || q.category || "General")
      || undefined,
  })

  const normalizeTitle = (title: string) => title.trim().replace(/\s+/g, " ").toLocaleLowerCase()
  const reserveCopyTitle = (title: string, reserved: Set<string>) => {
    let suffix = 1
    let candidate = `${title} (Copy)`
    while (reserved.has(normalizeTitle(candidate))) {
      suffix += 1
      candidate = `${title} (Copy ${suffix})`
    }
    reserved.add(normalizeTitle(candidate))
    return candidate
  }

  const findConflict = (title: string): string | null => {
    const norm = normalizeTitle(title)
    return quizzes.find(q => normalizeTitle(q.title) === norm)?.id || null
  }

  const buildJsonItems = (
    items: any[],
    settings?: JsonFileEntry["settings"],
    reservedTitles = new Set(jsonEntries.flatMap(entry => entry.quizzes).map(item => normalizeTitle(item.quiz.title))),
  ): JsonQuizItem[] => {
    const pendingTitles = reservedTitles
    return items.filter(q => q && typeof q === "object" && typeof q.title === "string" && Array.isArray(q.questions)).map(q => {
      const quiz = normalizeImportedQuiz(q, settings)
      const conflictId = findConflict(quiz.title)
      const normalizedTitle = normalizeTitle(quiz.title)
      const duplicateInBatch = pendingTitles.has(normalizedTitle)
      pendingTitles.add(normalizedTitle)
      const validation = validateQuizPayload(quiz)
      return {
        quiz,
        included: validation.ok && !duplicateInBatch,
        conflictId,
        conflictAction: "copy" as ConflictAction,
        warning: !validation.ok
          ? validation.error
          : duplicateInBatch
            ? "Duplicate title in this import batch — excluded automatically"
            : undefined,
      }
    })
  }
  ...
  const processJsonFiles = async (files: File[]) => {
    if (files.length > MAX_IMPORT_FILES) {
      toast.error(`Select at most ${MAX_IMPORT_FILES} JSON files at a time`)
      return
    }
    ...
    const batchTitles = new Set(jsonEntries.flatMap(item => item.quizzes).map(item => normalizeTitle(item.quiz.title)))
    for (const entry of newEntries) {
      setJsonEntries(prev => prev.map(e => e.id === entry.id ? { ...e, status: "parsing" as const } : e))
      try {
        if (entry.file!.size > MAX_IMPORT_FILE_BYTES) throw new Error("File is larger than 2 MB")
        const text = await entry.file!.text()
        const data = JSON.parse(text)
        const items: any[] = Array.isArray(data) ? data : [data]
        const quizItems = buildJsonItems(items, undefined, batchTitles)
        if (!quizItems.length) {
          setJsonEntries(prev => prev.map(e => e.id === entry.id ? { ...e, status: "error" as const, error: "No valid quizzes found in this file" } : e))
        } else {
          const firstQuiz = quizItems[0].quiz
          setJsonEntries(prev => prev.map(e => e.id === entry.id ? {
            ...e,
            status: "ready" as const,
            quizzes: quizItems,
            settings: {
              category: firstQuiz.category,
              section: firstQuiz.section || "General",
              difficulty: firstQuiz.difficulty || "medium",
              visibility: firstQuiz.visibility || "public",
            },
          } : e))
        }
      } catch (error) {
  ...
  const addPastedJsonEntry = () => {
    try {
      ...
      setJsonEntries(prev => [...prev, {
        ...
        settings: {
          category: quizItems[0].quiz.category,
          section: quizItems[0].quiz.section || "General",
          difficulty: quizItems[0].quiz.difficulty || "medium",
          visibility: quizItems[0].quiz.visibility || "public",
        },
        expanded: true,
      }])
  ...
  const toggleJsonQuiz = (fileId: string, idx: number) => {
    setJsonEntries(prev => prev.map(e => {
      ...
      if (quizzes[idx].warning) return e
  ...
  const importAllJsonQuizzes = async () => {
    const toImport: Quiz[] = []
    const toReplace: Quiz[] = []
    const reservedTitles = new Set(quizzes.map(quiz => normalizeTitle(quiz.title)))
    for (const entry of jsonEntries) {
      ...
      for (const item of entry.quizzes) {
        ...
        if (item.conflictId) {
          ...
          toImport.push({ ...item.quiz, title: reserveCopyTitle(item.quiz.title, reservedTitles) })
        } else {
          reservedTitles.add(normalizeTitle(item.quiz.title))
          toImport.push(item.quiz)
        }
      }
    }
    if (!toImport.length && !toReplace.length) { toast.error("No quizzes selected to import"); return }
    for (const quiz of [...toImport, ...toReplace]) {
      const validation = validateQuizPayload(quiz)
      if (!validation.ok) {
        toast.error(`${quiz.title}: ${validation.error}`)
        return
      }
    }
    ...
    if (!await saveQuizzes(updatedQuizzes)) return
    setMainTab("quizzes"); resetImportDialog()
  ...
  const ensureCategoryExists = async (category: string) => {
    ...
    try {
      ...
      toast.error(`Could not create category "${category}"`)
    }
  }
  ...
  const processMultipleFiles = async (files: File[]) => {
    if (files.length > MAX_IMPORT_FILES) {
      toast.error(`Select at most ${MAX_IMPORT_FILES} HTML files at a time`)
      return
    }
    const entries = files.map(f => makeHtmlEntry(f, f.name))
    setUploadEntries(prev => [...prev, ...entries])
    const batchTitles = new Set(uploadEntries.flatMap(item => item.quiz ? [normalizeTitle(item.quiz.title)] : []))
    for (const entry of entries) {
      ...
      try {
        ...
        if (quiz) {
          const normalizedTitle = normalizeTitle(quiz.title)
          const conflictId = findConflict(quiz.title)
          const duplicateInBatch = batchTitles.has(normalizedTitle)
          ...
          const finalQuiz = {
            ...
            structureLocation: quiz.structureLocation
              || findStructureLocationByCategoryName(contentFolders, quiz.category)
              || undefined,
          }
  ...
  const addPastedHtmlEntry = async () => {
    ...
    await ensureCategoryExists(quiz.category)
    const conflictId = findConflict(quiz.title)
    const duplicateInBatch = uploadEntries.some(entry => entry.quiz && normalizeTitle(entry.quiz.title) === normalizeTitle(quiz.title))
  ...
  const toggleHtmlEntry = (id: string) => setUploadEntries(prev => prev.map(e =>
    e.id === id && !e.warning ? { ...e, included: !e.included } : e
  ))
  ...
  const updateEntrySettings = (id: string, key: string, value: any) => {
    const structureLocation = key === "category"
      ? findStructureLocationByCategoryName(contentFolders, String(value)) ?? undefined
      : undefined
    setUploadEntries(prev => prev.map(e => {
      if (e.id !== id) return e
      return {
        ...
        settings: {
          ...e.settings,
          [key]: value,
          ...(key === "category" ? { structureLocation: structureLocation ?? { folderId: "", categoryId: "", sectionId: "" } } : {}),
        },
        quiz: e.quiz ? {
          ...e.quiz,
          [key]: value,
          ...(key === "category" ? { structureLocation } : {}),
        } : null,
      }
    }))
  }
  ...
  const importAllReady = async () => {
    ...
    for (const e of uploadEntries) {
      ...
      if (e.conflictId) {
        ...
        toImport.push({ ...e.quiz, title: reserveCopyTitle(e.quiz.title, reservedTitles) })
      } else {
        reservedTitles.add(normalizeTitle(e.quiz.title))
      ...
      {selectedQuizzes.size > 0 && (
        <div className="flex items-center gap-2 p-2 sm:p-3 bg-primary/5 border border-primary/20 rounded-lg flex-wrap">
          ...
          <div className="flex items-center gap-1 sm:gap-2 ml-auto flex-wrap">
            ...
            <Button aria-label="Clear selected quizzes" variant="ghost" size="sm" onClick={() => setSelectedQuizzes(new Set())} className="h-7 w-7 p-0"><X className="h-3 w-3" /></Button>
          </div>
        </div>
      )}
                  ...
                  <CardHeader className="pb-2 sm:pb-3 px-3 sm:px-6">
                    ...
                    <div className="flex items-center gap-2 mt-2 flex-wrap ml-6 sm:ml-0">
                      ...
                      <div className="ml-auto flex items-center gap-0.5">
                        <Button aria-label={`Copy link for ${quiz.title}`} size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => copyQuizLink(quiz.id)}><Copy className="h-3.5 w-3.5" /></Button>
                        <Button aria-label={`Open ${quiz.title}`} size="sm" variant="ghost" className="h-7 w-7 p-0" asChild><a href={`/quiz/${quiz.id}`} target="_blank"><ExternalLink className="h-3.5 w-3.5" /></a></Button>
                        <Button aria-label={`Edit ${quiz.title}`} size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleEditQuiz(quiz)}><Edit className="h-3.5 w-3.5" /></Button>
                        <Button aria-label={`Export ${quiz.title}`} size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => exportQuizJson(quiz)}><Download className="h-3.5 w-3.5" /></Button>
                        <Button aria-label={`Duplicate ${quiz.title}`} size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleDuplicateQuiz(quiz)}><Copy className="h-3.5 w-3.5 text-primary" /></Button>
                        <Button aria-label={`Delete ${quiz.title}`} size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleDeleteQuiz(quiz.id)}><Trash2 className="h-3.5 w-3.5 text-destructive" /></Button>
                      </div>
                    </div>
                  </CardHeader>
                          ...
                          {quiz.questions.map((q, idx) => (
                            <div key={q.id} className="flex items-center gap-2 p-2 sm:p-3 bg-muted/50 rounded-lg">
                              ...
                              <div className="flex flex-col gap-0.5 shrink-0">
                                <Button aria-label={`Move question ${idx + 1} up`} variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => handleMoveQuestion(quiz.id, idx, -1)} disabled={idx === 0}>
                                  <ArrowUp className="h-3 w-3" />
                                </Button>
                                <Button aria-label={`Move question ${idx + 1} down`} variant="ghost" size="sm" className="h-5 w-5 p-0" onClick={() => handleMoveQuestion(quiz.id, idx, 1)} disabled={idx === quiz.questions.length - 1}>
                              ...
                              <Button aria-label={`Edit question ${idx + 1}`} size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleEditQuestion(quiz.id, q)}><Edit className="h-3 w-3" /></Button>
                              <Button aria-label={`Delete question ${idx + 1}`} size="sm" variant="ghost" className="h-7 w-7 p-0" onClick={() => handleDeleteQuestion(quiz.id, q.id)}><Trash2 className="h-3 w-3 text-destructive" /></Button>
                            </div>
                          ))}
                        </div>
                      )}
                ...
                <TabsContent value="basic" className="mt-3 space-y-3 sm:space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <Select value={editingQuiz.category} onValueChange={updateEditingQuizCategory}>
                        ...
                        <div className="p-4 pt-0 space-y-4 border-t border-border/50">
                          <div className="grid sm:grid-cols-2 gap-4">
                            <div className="space-y-2">
                              ...
                              <Select value={globalSettings.category} onValueChange={updateGlobalQuizCategory}>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-1.5">
                        <Button variant="ghost" size="sm" className="h-6 text-[10px] px-2"
                          onClick={() => setUploadEntries(prev => prev.map(e => ({
                            ...e,
                            included: e.status === "ready" && !e.warning,
                          })))}>All</Button>
                        <Button variant="ghost" size="sm" className="h-6 text-[10px] px-2"
                          ...
                          <div key={entry.id} className={`rounded-lg border overflow-hidden transition-all ${
                            ...
                            <div className="flex items-center gap-2 p-2.5">
                              <Checkbox
                                ...
                                disabled={Boolean(entry.warning)}
                                onCheckedChange={() => toggleHtmlEntry(entry.id)}
                                className="shrink-0"
                              />
                                        ...
                                        <div className="flex items-start gap-2.5">
                                          ...
                                          <div className="flex-1 min-w-0">
                                            <div className="flex items-center gap-1.5 flex-wrap">
                                              ...
                                              {item.warning && (
                                                <Badge variant="outline" className="text-[9px] px-1 h-4 border-red-400 text-red-600 gap-0.5 shrink-0">
                                                  <AlertCircle className="h-2.5 w-2.5" /> Auto-excluded
                                                </Badge>
                                              )}
                                            </div>
                                            {item.warning && (
                                              <p className="mt-1 text-[10px] text-red-600 dark:text-red-400">{item.warning}</p>
                                            )}
                ...
                <Button variant="destructive" size="sm" className="gap-1.5 text-xs hidden sm:flex"
                  onClick={() => {
                    ...
                    fetch("/api/quiz-results", { method: "DELETE", headers: adminHeaders() })
                      ...
                      .then(({ ok, data }) => {
                        if (ok && data?.success) { setLeaderboard([]); toast.success("Leaderboard cleared") }
                        else toast.error(data?.error || "Could not clear leaderboard")
                      }).catch(() => toast.error("Could not clear leaderboard"))
                  }}>
              ...
              {leaderboard.map((entry, idx) => (
                <div key={entry.id} className={`flex items-center gap-3 p-3 rounded-lg border ${idx===0?"bg-yellow-500/10 border-yellow-500/30":idx===1?"bg-gray-400/10 border-gray-400/30":idx===2?"bg-amber-700/10 border-amber-700/30":"bg-card border-border/50"}`}>
                  ...
                  <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-destructive hover:bg-destructive/10"
                    aria-label={`Delete leaderboard entry for ${entry.name}`}
                    onClick={() => {
                      fetch(`/api/quiz-results/${entry.id}`, { method: "DELETE", headers: adminHeaders() })
                        .then(async r => ({ ok: r.ok, data: await r.json().catch(() => null) }))
                        .then(({ ok, data }) => {
                          if (ok && data?.success) { setLeaderboard(prev => prev.filter(e=>e.id!==entry.id)); toast.success("Entry deleted") }
                          else toast.error(data?.error || "Could not delete entry")
                        })
