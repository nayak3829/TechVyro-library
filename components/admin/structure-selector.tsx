...
export function StructureSelector({ value, onChange, placeholder = "Select location", className, onCreateStructure }: StructureSelectorProps) {
  ...
  useEffect(() => {
    if (!open || !value?.folderId) return
    setExpandedFolders(prev => new Set(prev).add(value.folderId))
    if (value.categoryId) setExpandedCategories(prev => new Set(prev).add(value.categoryId))
  }, [open, value?.folderId, value?.categoryId])
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-80 p-0" align="start">
        <div className="p-2 border-b">
          ...
          <p className="text-[10px] text-muted-foreground">Expand a folder and category, then select a section</p>
        </div>
