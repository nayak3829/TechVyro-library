...
import { fetchWithTimeout, readBoundedJson, RequestBodyError, summaryRequestSchema } from "@/lib/ai-request-security"
...
export async function POST(request: Request) {
  ...
  try {
    const parsedRequest = summaryRequestSchema.safeParse(await readBoundedJson(request))
    if (!parsedRequest.success) {
      return NextResponse.json({ error: "Invalid summary request" }, { status: 400 })
    }
    const { title, description, category } = parsedRequest.data
