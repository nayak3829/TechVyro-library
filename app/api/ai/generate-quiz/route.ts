...
import { fetchWithTimeout, quizRequestSchema, readBoundedJson, RequestBodyError } from "@/lib/ai-request-security"
...
export async function POST(request: Request) {
  ...
  try {
    const parsedRequest = quizRequestSchema.safeParse(await readBoundedJson(request))
    if (!parsedRequest.success) {
      return NextResponse.json({ error: "Invalid quiz request" }, { status: 400 })
    }
    const { topic, category, count, difficulty } = parsedRequest.data
