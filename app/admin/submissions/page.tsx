...
export default function AdminSubmissionsPage() {
  ...
  const closeDetail = useCallback(() => { setDetail(null); setDuplicate(null) }, [])
  ...
  const load = useCallback(async (nextOffset = 0) => {
    ...
    try {
      const response = await fetch(`/api/admin/submissions?status=${status}&limit=${PAGE_SIZE}&offset=${nextOffset}`)
  ...
  const changeStatus = (nextStatus: Status) => { if (nextStatus !== status) { setOffset(0); setSelected([]); setStatus(nextStatus) } }
