...
function SeriesContent() {
  ...
  const platform = searchParams.get("platform") || seriesTitle || "unknown"
  const category = searchParams.get("category") || "general"
  const location = searchParams.get("location") || "series_page"
  ...
  const startTest = (test: Test) => {
    ...
    if (!user) {
      router.push(loginHref(destination))
      return
    }
                ...
                {!user && (
                  <div className="mt-3 flex items-center gap-2 bg-primary-foreground/10 border border-primary-foreground/20 rounded-lg px-3 py-2 text-xs">
                    ...
                    <span className="text-primary-foreground/90">
                      <Link href={loginHref(`/test-series/series?${searchParams.toString()}`)} className="font-semibold underline">Login</Link> to start a test and save your progress
                    </span>
                  </div>
                )}
              </>
            ...
            <div className="max-w-5xl mx-auto px-4 py-3">
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <div className="flex gap-2">
                  ...
                  <div className="flex rounded-lg border border-border overflow-hidden" role="group" aria-label="Series results layout">
                    <button
                      type="button"
                      ...
                      {subjects.map((subj, idx) => {
                        ...
                        return (
                          <Card key={subjId} className="overflow-hidden">
                            <button
                              type="button"
                              aria-expanded={!!expanded[subjId]}
                              aria-controls={`subject-tests-${subjId}`}
                              aria-label={`${expanded[subjId] ? "Collapse" : "Expand"} ${name} subject`}
                            ...
                            {expanded[subjId] && (
                              <div className="border-t" id={`subject-tests-${subjId}`}>
                                {subjTests.length === 0 ? (
