...
export default function BrowsePage() {
  ...
  const fetchAll = useCallback(async (initial = false) => {
    ...
    try {
      const [pdfRes, quizRes, structRes] = await Promise.all([
        fetch("/api/pdfs", { signal: controller.signal }),
  ...
  useEffect(() => {
    ...
    const refresh = (initial: boolean) => {
      // fetchAll owns its request; this guard prevents an interval callback after unmount.
      if (active) fetchAll(initial)
    }
    refresh(true)
    const iv = setInterval(() => refresh(false), REFRESH_MS)
    return () => {
      active = false
      clearInterval(iv)
      refreshControllerRef.current?.abort()
  ...
  const handleTabKeyDown = (event: React.KeyboardEvent<HTMLButtonElement>, index: number) => {
    ...
    if (event.key === "ArrowLeft" || event.key === "ArrowUp") nextIndex = (index + 2) % 3
    if (event.key === "Home") nextIndex = 0
    if (event.key === "End") nextIndex = 2
    if (nextIndex !== undefined) {
      event.preventDefault()
      const nextTab = (["all", "pdfs", "quizzes"] as Tab[])[nextIndex]
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex flex-col gap-3 mb-5">
              <div className="flex items-center gap-2">
                {/* Tab pills */}
                <div className="flex gap-1 p-1 rounded-xl bg-muted/50 border border-border/50 h-fit" role="tablist" aria-label="Browse content type">
                  {(["all", "pdfs", "quizzes"] as Tab[]).map((tab, index) => (
                    <button
                      key={tab}
                    ...
                    <div className="grid grid-cols-1 xs:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3 sm:gap-4">
                      {filteredPdfs.map(pdf => {
                        const color = pdf.category?.color || CATEGORY_COLORS[pdf.category?.name || ""] || "#6366f1"
                            const isNew = now !== null && (now - new Date(pdf.created_at).getTime()) < 7 * 24 * 60 * 60 * 1000
                        return (
                          <div key={pdf.id} className="group rounded-2xl border border-border/50 bg-card hover:shadow-xl hover:-translate-y-1 transition-all duration-300 flex flex-col overflow-hidden">
                            ...
                            <div className="p-4 flex-1 flex flex-col">
                              {pdf.thumbnail_url && (
                                <div className="-mx-4 -mt-4 mb-3 h-32 overflow-hidden bg-muted/40 p-2">
                                  <img
